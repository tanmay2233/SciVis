...
for day in range(1, num_days + 1):
    ...
    # Loop through latitude lines and select 50 points at uniform intervals
    ...
    norm = Normalize(vmin=-0.5, vmax=20)
    ...
    # Set the same constant magnitude for all arrows
    arrow_magnitude = 10.0

    # Calculate the x and y components of the arrows based on magnitude and direction
    selected_zonal = arrow_magnitude * \
        np.cos(direction[selected_y, selected_x])
    selected_meridional = arrow_magnitude * \
        np.sin(direction[selected_y, selected_x])

    # Create the quiver plot with constant-sized arrows for all selected points
    plt.quiver(selected_x, selected_y, selected_zonal, selected_meridional,
               angles='xy', scale_units='xy', scale=1, color='black', headwidth=2, headlength=3)
